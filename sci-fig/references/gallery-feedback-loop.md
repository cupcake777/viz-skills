...
Location: `/root/ops/plotting/gallery_feedback.jsonl`
...
1. **Read feedback**: `cat /root/ops/plotting/gallery_feedback.jsonl | python3 -c "import sys,json; [print(json.dumps(json.loads(l))) for l in sys.stdin]"`
2. **For each entry**:
   ...
   - `suggest` → **This is the actionable feedback loop**:
     ...
     b. Modify the corresponding template script in `/root/ops/plotting/templates/`
     c. Re-render: `python3 /root/ops/plotting/templates/{chart}.py`
     d. Verify the fix visually
     e. Notify user for re-confirmation
3. **After processing**: Optionally truncate the file `> /root/ops/plotting/gallery_feedback.jsonl`
...
| "字体太小" | Increase `fontsize` parameter (default 5→7pt for Manhattan, 6→7pt for Volcano) |
