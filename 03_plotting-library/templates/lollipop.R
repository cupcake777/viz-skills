...
generate_mock_data <- function(seed = 42) {
  ...
  list(df=sites, domains=domains, protein_length=393)
}
...
if (sys.nframe()==0) { d<-generate_mock_data(); save_demo(lollipop_plot(d$df,d$domains,d$protein_length), "lollipop_demo", width=120, height=70) }