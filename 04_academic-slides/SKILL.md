...
学术报告/答辩专用的叙事与版式层。它负责“这页讲什么、怎么排、图如何服务论点”，不负责底层 PPTX 文件操作，也不负责具体绘图代码。

在 `viz-skills` 架构里：
- `academic-slides` = 学术叙事、页面密度、图文布局
- `powerpoint` = `.pptx` 读取、编辑、生成、验证
- `sci-fig` = 科学图的选图、审美规范、QA
- `plotting-library` = 可执行绘图模板，静态科研图默认优先 R

## 架构

```
academic-slides (skill)    ← 你现在读的：学术PPT叙事+版式+图文排版
    │
    ├── powerpoint         ← 底层PPTX文件操作（pptxgenjs/python-pptx/OOXML）
    ├── sci-fig            ← figure决策与质量标准
    └── plotting-library   ← R-first可执行绘图模板库
```

**关键原则：PPT不是把论文图整张贴进去。先定义每页论点，再决定图是否需要重绘、裁剪或改为presentation版。图的生成与修改走`sci-fig`/`plotting-library`，PPT文件落地走`powerpoint`。**

## 触发与协作边界

| 用户需求 | 主 skill | 协作 |
|----------|----------|------|
| 做组会/答辩/论文汇报PPT | academic-slides | powerpoint |
| 把一组figure整理成slides | academic-slides | sci-fig, plotting-library, powerpoint |
| 修改已有`.pptx`文件内容或版式 | powerpoint | academic-slides |
| 某张图放PPT里看不清 | academic-slides | sci-fig |
| 需要重新出PPT尺寸数据图 | sci-fig | plotting-library |

跨 skill 工作流见 `../01_workflows/figure-to-slide.md`。
...
2. 使用`plotting-library`里的`volcano.R`模板
